 ...
 */
function applyAdjustment(date, sign, value, unit) {
 ...
 */
function applyRounding(date, unit) {
    switch (unit) {
        case 'd':
 ...
 */
exports.parse = function parse(datestring) {
 ...
 */
exports.stringify = function stringify(date) {
    ...
    if (date.getUTCDay() === 0 && date.getUTCHours() === 0 &&
        date.getUTCMinutes() === 0 && date.getUTCSeconds() === 0 &&
        date.getUTCMilliseconds() === 0) {
        timeDifferences = date.getTime() - convertToDateOnly(nowUTC).getTime();
    ...
    // Check if it matches min precision
    if (date.getUTCSeconds() === 0 && 
       date.getUTCMilliseconds() === 0) {
       const hours = Math.floor(timeDifference / msInHour);
       return hours !== 0?`now${sign}${hours}m`:`now/m`;
   }
